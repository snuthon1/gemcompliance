 ...
 * @param {string} docType - 'PAN_CARD' | 'GST_CERT' | 'UDYAM_CERT'
...
export async function scanAndVerifyDocument(file, docType, bidder, onProgress) {
  ...
  const hasReadableText = cleanText.length >= 10;
  ...
  if (docType === 'PAN_CARD') {
    ...
    if (detectedPans.length === 0) {
      ...
      // Check if one of the detected PANs matches the bidder's PAN
