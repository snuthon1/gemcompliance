...
export default function BidderDetail() {
  ...
  const handleDocUpload = async (e) => {
    ...
    setUploading(true);
    setUploadMsg({ type: 'info', text: 'Scanning document text via OCR...' });
    ...
    try {
      scanResult = await scanAndVerifyDocument(
        ...
        uploadDocType,
        bidder,
        (pct, msg) => setUploadMsg({ type: 'info', text: `${msg} (${pct}%)` })
      );
    } catch (scanErr) {
      ...
      if (data.success) {
        setUploadMsg({
          ...
          text: data.document?.flagged
            ? `⚠️ Document audited with notice: ${data.document?.flag_reason}`
            : '✅ Document verified clean and entered into vault!'
        });
        setUploadFile(null);
                ...
                return (
                  ...
                  >
                    <div className="flex items-start justify-between gap-2">
                      <div className="flex items-center space-x-2.5 min-w-0">
                        <div className={`w-9 h-9 rounded-xl flex items-center justify-center font-bold text-xs shrink-0 ${
                          isFlagged ? 'bg-rose-100 text-rose-700' : 'bg-slate-100 text-[#0B2546]'
                        }`}>
                        ...
                        <div className="min-w-0">
                          ...
                          <div className="flex items-center space-x-2 text-[11px] text-slate-400 mt-0.5">
                            <span>Uploaded {new Date(doc.uploaded_at).toLocaleDateString('en-IN', { day: 'numeric', month: 'short', year: 'numeric' })}</span>
                            {extData?.identified_number && (
                              <>
                                <span>&bull;</span>
                                <span className="font-mono text-slate-700 font-semibold">
                                  ID: {extData.identified_number}
                                </span>
                              </>
                            )}
                          </div>
                    ...
                    {isFlagged && doc.flag_reason && (
                      <div className="p-2.5 rounded-xl bg-rose-50 border border-rose-200 text-rose-800 text-[11px] flex items-start space-x-2">
